...
async function getGitHubReleaseTag() {
  try {
    // Fetch from GitHub API to get the latest release tag
    ...
    console.error('Error fetching GitHub release tag:', error)
    return 'unknown version' // Fallback if API call fails
  }
}
...
export async function exportToPdf(
  ...
  const releaseTag = await getGitHubReleaseTag()
  ...
  try {
    ...
    let baseFilename = 'AI_verordening_beslishulp'

    if (filename && filename.trim() !== '') {
      // Format with filename: AI_verordening_beslishulp_NAME_DATE.pdf
      baseFilename = `AI_verordening_beslishulp_${filename}`
    }
    // Create the actual filename with date
    const actualFilename = `${baseFilename}_${isoDate}.pdf`
